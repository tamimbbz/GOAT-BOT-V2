module.exports.config = {
    ...
    role: 2,
    author: "xalman",
    description: "150+ Non-stop Banglish tag list (3.5s gap)",
    category: "nsfw",
...
module.exports.onStart = async function({ api, event, args }) {
    ...
    if (!targetID) return api.sendMessage("Bos, jare chudben tare mention den!", threadID, messageID);

    const userInfo = await api.getUserInfo(targetID);
    name = userInfo[targetID].name;
    const arraytag = [{ id: targetID, tag: name }];

    api.sendMessage(`Target Locked: ${name}\nEbar 150 shoter dhamaka dekh!`, threadID);
    ...
    const insults = [
        ...
        "Tor maire chude bucket vore mal out korbo.", "Tor ammu re rasta te nengto kore chudmu.",
        "Tor boner vuday kukur er baccha jonmo dawamu.", "Tor maire chude lungi fira banamu maderchod.",
        "Tor ammur vuday gorom teler pitha banamu.", "Tor boner vuday dhon dhukale kemon lage re?",
        "Tor maire chude ammer tel banamu magir pola.", "Tor ammu re bazarer manush diye chudamu.",
        "Tor boner vuday kaktarua dhukaya dimu.", "Tor maire chude pitha banamu maderchod.",
        "Tor ammur vuday bash diya khuchamu.", "Tor boner vuday kukur er mal out korbo.",
        "Tor maire chudte chudte paharer upor niye jamu.", "Tor ammur vuday petrol dhele agun jalaya dimu.",
        "Tor boner vuday lathi diya khuciye dimu.", "Tor maire chude dhaka sohor ghuramu khankir po.",
        "Tor ammur vuday ekhon o dhon dukhle tui rag korish.", "Tor boner kochi vuday kukur er mal out korbo.",
        "Tor maire chude bucket vore mal ber korbo.", "Tor ammu re bus stand e line dhoraya chudmu.",
        "Khankir pola tor boner vuday moricher gura dimu.", "Kire jaroj tor bap ki ekhono rasta te vaat khay?",
        "Tor boner kochi vuday amar gari chalamu.", "Tor ammur vuday pichon diya bash dukamu.",
        "Tor maire chude achar banamu ami maderchod.", "Tor boner vuday kaktarua dhukaya dimu.",
        "Tor ammu re launch er cabin e chudche amar bondhura.", "Tor boner hoda diya rasta banamu maderchod.",
        "Tor ammur kala vuday amar lal dhon dukamu ekhoni.", "Tor maire chudte chudte chader deshe niye jamu.",
        "Tor ammu re gorur gari te tule chudmu gramer moddhe.", "Maderchod tor bon ke mela te niye chude aslam.",
    ...
    setTimeout(() => {
        api.sendMessage("~ 150 Shot Over! Tor poribar uddhar hoye geche, ebar matha nichu kore thak!", threadID);
    }, delay);
};
